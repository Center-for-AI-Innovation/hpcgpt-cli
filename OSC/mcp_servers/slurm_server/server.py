...
class SlurmMCP(FastMCP):
    ...
    async def accounts(self) -> str:
        """
        Run the OSCprojects command and return the output.

        Args:
            username: The system username to run the OSCprojects command for.

        Returns:
            The output of the OSCprojects command.
        """
        result = subprocess.run(["OSCprojects"], capture_output=True, text=True)
        return result.stdout
...
def parse_command_line() -> argparse.Namespace:
    ...
    parser.add_argument("-t","--transport",
        ...
        choices = ['streamable-http', 'stdio', 'http', 'sse'],
    )
