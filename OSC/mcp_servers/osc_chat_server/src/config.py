...
class Config(BaseModel):
    ...
    osc_chat_url: str = Field(
        description="The URL of the OSC Chat API, can also be set with the OSC_CHAT_URL environment variable",
        json_schema_extra={"env": "OSC_CHAT_URL"})
    osc_chat_api_key: str = Field(
        description="API key for the OSC Chat API, can also be set with the OSC_CHAT_API_KEY environment variable",
        json_schema_extra={"env": "OSC_CHAT_API_KEY"})
    osc_chat_model: str = Field(
        description="The model to use for the OSC Chat API, can also be set with the OSC_CHAT_MODEL environment variable",
        json_schema_extra={"env": "OSC_CHAT_MODEL"})
    osc_chat_system_prompt: str = Field(
        default=DEFAULT_SYSTEM_PROMPT,
        description="System message prepended to each OSC Chat API request",
    )
